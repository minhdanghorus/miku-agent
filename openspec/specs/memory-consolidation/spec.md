...
An explicit pass that resolves what the store has accumulated.

Facts arrive one at a time and never argue with each other. Over enough turns a store holds
corrections, restatements, fragments of one preference, and windows that have closed - and
every one of them still rides along in every turn. Consolidation is the step that says which
of them is still true.

It runs on demand and never inside a turn. A pass that fired on a threshold would make some
turns silently cost several times what the others cost, and would put a store write on a path
the user did not ask for. The model proposes a plan and code applies it: the model is never
handed a tool that writes, so an odd plan costs a dropped operation rather than a damaged
store.

Nothing is destroyed. A resolved fact keeps its row and its original text, and carries a marker
naming what replaced it - so the pass is auditable after the fact, recall has one field to
filter on, and a second run over the same facts has nothing left to do.
