...
What a stored conversation is when it is read back: how conversations are enumerated and
ordered, how one is named and sized, which stored messages are shown and which are
structural, how one is resumed, and what removing one does and does not reach.

The data has been there since the first phase - thread state is keyed by conversation
identifier and nothing else - so this capability adds no table, no column, and no field to
any event. What it defines is the reading.

Two things are load-bearing. A transcript reports what was said, never how the graph library
stored it, because that format belongs to a library and moves on its schedule. And removal
states its own boundary: it reaches thread state alone, while facts keyed by user and traces
keyed by turn survive it, so the interface says so before the removal rather than after.
