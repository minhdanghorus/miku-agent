...
One allowance per turn, counting model requests.

Fan-out multiplies breadth by depth: a loop free to call a delegating tool on every iteration
has no bound worth the name unless something counts across both. The budget is that counter -
created for the turn, shared by reference with anything the turn delegates to, and never shared
between turns, including turns running concurrently in one process.

It is not model-facing. A limit the model can read is a limit the model can argue with, so the
budget reaches tools through the invocation configuration and appears in no tool schema.
Exhaustion stops the work and is recorded; the turn still replies.
