...
The seam by which tools this repository did not write become tools the agent can call: Model
Context Protocol servers, described in a file, connected while a session opens, and offered to the
model beside the built-in ones.

It exists so that adding a capability is an edit to configuration rather than a change to source,
in the same way that adding a provider is a descriptor. The loop stays unaware of where a tool came
from - a contributed tool is looked up by name and invoked through the same path a built-in one is.

The connector is inactive unless it is both enabled and configured, because a forgotten
configuration file spawns processes on every session and fails silently. What it reaches is
deliberately narrow: tools only, one transport, text results only, an allowlist rather than a
denylist. A broken server degrades to a warning and a session that still opens, because errors in
this system degrade rather than crash.
