...
long-term memory holds, and what a past turn did.

It exists so that a gateway never reads those sources itself. Configuration resolution stays
in one place, inspection receives its configuration and store handle as arguments, and every
absent thing - an empty store, a date with no trace file, an unknown turn - is reported as
