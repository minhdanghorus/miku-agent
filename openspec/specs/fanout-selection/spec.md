...
Best-of-N as a delegated subgraph: several branches draft candidate answers at once, and one
step picks among them.

Diversity here is structural, not sampled. Each branch is handed a distinct angle held in code,
because a temperature knob gives no account of why two candidates differ and nothing an
evaluator can assert on. Angles are recorded, so the shape of a fan-out turn is readable from
its trace alone.

The subgraph hangs off a tool rather than off the main loop. Delegation therefore costs the
loop nothing - it is still the same three nodes - and a partial fan-out still answers: selection
runs over whatever candidates arrived rather than failing because one branch did not.
